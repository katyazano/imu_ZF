...
// Agregamos los iconos nuevos para la barra inferior
import { Bell, User, Search, Camera, LogOut, Home, QrCode, FileText, ClipboardList, CheckSquare } from 'lucide-react';
...
const Navbar = () => {
  ...
  // 4. ENLACES DINÁMICOS POR ROL (Escritorio)
  const getNavLinks = () => {
    switch (rolActivo) {
  ...
  // 5. CONFIGURACIÓN DEL BOTÓN SECUNDARIO MÓVIL
  ...
  return (
    <>
      {/* ==============================================================
          💻 TU NAVBAR ORIGINAL DE ESCRITORIO (Ahora sin el drawer)
          ============================================================== */}
      <nav className="bg-[#0070BC] text-white relative font-sans shadow-md z-50">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          
          <div className="flex items-center gap-4">
            {/* Quitamos el botón de hamburguesa y dejamos que el logo se vea siempre */}
      ...
      {/* ==============================================================
          📱 NAVBAR INFERIOR PARA MÓVIL (Solo se ve en celulares)
          ============================================================== */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 flex justify-around items-center h-[72px] z-[100] pb-2 px-2 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] rounded-t-[30px]">
        ...
        {/* Botón Inicio */}
        <button onClick={handleHomeClick} className={`flex flex-col items-center justify-center w-full h-full pt-2 transition-all active:scale-95 ${isActive(getHomePath()) ? 'text-[#0070BC]' : 'text-gray-400 hover:text-gray-600'}`}>
        ...
        {/* Botón Dinámico por Rol (Validar, Trámites, etc) */}
        <button onClick={() => navigate(mobileSecondary.path)} className={`flex flex-col items-center justify-center w-full h-full pt-2 transition-all active:scale-95 ${isActive(mobileSecondary.path) ? 'text-[#0070BC]' : 'text-gray-400 hover:text-gray-600'}`}>
        ...
        <button onClick={() => navigate('/scanner')} className="relative flex flex-col items-center justify-center w-full h-full active:scale-90 transition-transform">
          ...
          <span className="text-[9px] font-black uppercase tracking-widest mt-10 text-gray-500">Escanear</span>
        </button>

        {/* Botón Perfil */}
        <button onClick={() => navigate('/perfil')} className={`flex flex-col items-center justify-center w-full h-full pt-2 transition-all active:scale-95 ${isActive('/perfil') ? 'text-[#0070BC]' : 'text-gray-400 hover:text-gray-600'}`}>
