...
import { ArrowLeft, Clock, Loader2, Package, X, ChevronLeft, ChevronRight } from 'lucide-react';
...
const MisSolicitudes = () => {
  ...
  // Reseteamos a página 1 si el usuario busca algo nuevo o cambia de pestaña
  useEffect(() => {
    setPaginaActual(1);
  }, [filtro, query]);
  ...
  useEffect(() => {
    const fetchMisSolicitudes = async () => {
      try {
        ...
        // Armamos la URL inteligente
        const queryParms = new URLSearchParams();
        queryParms.append('page', paginaActual);
        queryParms.append('limit', 10); // Mostramos 10 solicitudes por página
        queryParms.append('estatus', filtro);
        if (query) queryParms.append('q', query);
        
        const response = await fetch(`${baseUrl}/solicitudes/mis?${queryParms.toString()}`, {
        ...
        // Guardamos las páginas totales
        if (rawData.meta) {
          setTotalPaginas(rawData.meta.totalPaginas);
        }
      } catch (error) {
      ...
      <main className="px-6 mt-6 space-y-4 flex-1">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Consultando...</p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            {totalPaginas > 1 && (
              <div className="flex justify-between items-center mt-4 p-4 bg-gray-50 rounded-2xl">
                <button
