...
const ScannerPage = () => {
  ...
  // Nuevo estado para que el guardia elija qué está haciendo
  ...
  const handleScanLogic = async (scannedQr) => {
    ...
    if (rolActivo !== 6) {
      if (rolActivo === 7 || rolActivo === 3) {
        navigate(`/auditor/trazabilidad/${scannedQr}`);
      } else {
        navigate(`/activo/${scannedQr}`);
      }
      return;
    }

    // 2. Seguridad (Rol 6) - Ejecuta el registro en bitácora
  ...
  const procesarEscaneoGuardia = async (qrCodigo) => {
    ...
    try {
      ...
      // Elegimos el endpoint basado en el modo seleccionado por el guardia
      ...
      const response = await fetch(`${baseUrl}${endpoint}`, {
        ...
        body: JSON.stringify({ qr_codigo: qrCodigo }) // Mandamos el QR en el body
      });

      const data = await response.json();

      if (response.ok) {
        setSuccessMsg(data.mensaje); // "Salida autorizada..." o "Equipo devuelto..."
        setStatus('approved');
      } else {
  ...
  {status === 'scanning' && (
    <>
      {/* 1. EL MOTOR DEL SCANNER (Fondo absoluto) */}
      ...
      {/* 2. HEADER FLOTANTE: BOTÓN DE RETROCESO Y SELECTOR DE MODO (Top absoluto) */}
      <div className="absolute top-20 md:top-24 left-0 right-0 px-6 z-20 flex justify-between items-center">
          {rolActivo !== 6 && (
          <button 
            onClick={() => navigate(-1)} 
          ...
          {rolActivo === 6 && (
            <div className="flex bg-black/50 backdrop-blur-xl p-1.5 rounded-full border border-white/10 shadow-2xl">
              <button 
                ...
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${scanMode === 'checkout' ? 'bg-[#0070BC] text-white' : 'text-gray-400 hover:text-white'}`}
              >
                <LogOut size={16} /> Salida
              </button>
              <button 
                onClick={() => setScanMode('checkin')}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${scanMode === 'checkin' ? 'bg-[#28B4AD] text-white' : 'text-gray-400 hover:text-white'}`}
              >
      ...
      {/* 3. MAGIA FLEXBOX: Este div invisible empuja todo lo demás hacia abajo */}
      <div className="flex-1 z-10 pointer-events-none"></div>

      {/* 4. CONTENEDOR INFERIOR FLEXIBLE (Leyenda y Simulación) */}
      <div className="z-30 flex flex-col items-center gap-4 px-4 w-full pb-[110px] lg:pb-12 pointer-events-none">
        ...
        {/* BOTÓN DE SIMULACIÓN */}
        <button 
          onClick={() => handleScanLogic('24d1c271-ec46-4701-ba45-d36461ed86d0')} 
